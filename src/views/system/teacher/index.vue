        ...
        <el-card shadow="never">
            ...
            <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 260px)"
                ...
                <template slot-scope="{index}">
                    ...
                    <el-table-column prop="user.name" label="教师名称" sortable="custom" show-overflow-tooltip min-width="120"
                        align="center" />
                    <el-table-column prop="user.email" label="教师邮箱" sortable="custom" show-overflow-tooltip min-width="120"
                        align="center" />
                    <el-table-column prop="status" label="关注" width="75px" sortable="custom" :resizable="false">
                        <template slot-scope="{row}">
                            <el-switch v-model="row.follow" @change="follow(row)" :active-value="1"
                                :inactive-value="2" />
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" width="220px" align="center" :resizable="false" fixed="right">
                        <template slot-scope="{row}">
                            <router-link :to="'/system/chat?id=' + row.user.id">发送消息</router-link>
                        </template>
                    </el-table-column>
                </template>
            </ele-data-table>
...
export default {
    ...
    data() {
        return {
            table: { url: '/user/teachers', where: {} },  // 表格配置
            choose: [],  // 表格选中数据
    ...
    methods: {
        ...
        follow(row) {
            ...
            this.$http.post(`/user/students/${row.user.id}/follow`).then(res => {
                loading.close();
                if (res.status === 200) {
