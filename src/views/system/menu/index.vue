...
export default {
  ...
  methods: {
    ...
    query() {
      this.loading = true;
      this.$http.get('/menu', {params: this.where}).then(res => {
        this.loading = false;
        if (res.data.code === 200) {
          this.data = this.$util.toTreeData(res.data.data.list, 'id', 'pid');
        } else {
    ...
    /* 显示编辑 */
    edit(row) {
      this.$http.get(`/menu/${row.id}`).then(res => {
        if (res.data.code === 200) {
